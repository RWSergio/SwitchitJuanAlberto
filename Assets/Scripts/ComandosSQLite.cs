...
public class ComandosSQLite : MonoBehaviour
{
    ...
    public string DBFileName = "baseDatos.db";
    //Variable texto UI
    //public Text myText;
    ...
    public void InsertarDatosNivel(int muertes, int moneda1, int moneda2, int moneda3)
    {
        AbrirDB();
        string nombreNivel = SceneManager.GetActiveScene().name;
        // Crear la consulta
        dbCommand = dbConnection.CreateCommand();
        string sqlQuery = String.Format("INSERT INTO \"{0}\"(Muertes, Moneda1, Moneda2, Moneda3) values(\"{1}\", \"{2}\", \"{3}\", \"{4}\")", nombreNivel, muertes, moneda1, moneda2, moneda3); //String.Format podemos pasarle datos a una cadena de caracteres para que tome el argumento
        dbCommand.CommandText = sqlQuery;
    ...
    void INSERT(string dato)
    {
        // Crear la consulta
        dbCommand = dbConnection.CreateCommand();
        string sqlQuery = String.Format("INSERT INTO Registro(Nombre) values(\"{0}\")", dato); //String.Format podemos pasarle datos a una cadena de caracteres para que tome el argumento
        dbCommand.CommandText = sqlQuery;
        //Para poder contar datos de las filas tenemos que poner
        dbCommand.ExecuteScalar();
        //ExecuteScalar me devuelve un objeto 
